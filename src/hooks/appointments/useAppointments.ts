...
let inMemoryStore: ConsultationRequest[] = mockConsultations.map((cons) => {
    ...
    // [FIX]: Thay vì 'any', ta ép kiểu về Record các field có thể là string hoặc undefined
    // Điều này an toàn hơn và thỏa mãn ESLint
    const rawCons = cons as unknown as Record<string, string | undefined>;

    // Xác định trạng thái và ghi chú ban đầu
    const initialStatus = (rawCons.status as ConsultationStatus) || 'PENDING';
    
    // Logic lấy note: ưu tiên 'note', nếu không có thì lấy 'notes', cuối cùng là mặc định
